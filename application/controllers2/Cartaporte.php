...
//$cond="cartaporte.carid>0 ";
    ...
    if ($estatus=="Pendientes"){
      $cond="and (cartaporte.tipo='PendC' or cartaporte.Terminada='N') ";
    }else{
...
//$conduser='';
  ...
  $campos="cartaporte.carid, DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha , cartaporte.destino, cartaporte.impresa, cartaporte.tipo, left(clientes.nombre,15) as nombre, left(transportistas.nombre,15) as nombretrans, transchofer.nombrechofer as chofer, cartaporte.tipofactcp, cartaporte.serie, cartaporte.facidn, cartaporte.numfac, cartaporte.timbrada, cartaporte.cliid ";
...
/************************************************************************/
 public function seleid_post(){
  ...
  $id=$this->datap['id'];
  $xsql ="Select carid, cliid, carnum, destino, fecha, dircliid, traid, equipidt, equipidr, choid, plaid, empreid, userid as empid, ruta, tipofactcp, facidn, numfac  from cartaporte where carid=$id";  
  $query = $this->db->query($xsql);
    if ($query) {
      $respuesta = array(
        'error' => FALSE,            
...
//  $xsql ="Select a.carid, a.cliid, a.carnum, a.dircliid, a.traid, b.nombre, c.descripcion, d.nombre as nombretrans, a.equipidt, e.descripequip, a.plaid, f.placas, a.choid, g.nombrechofer from cartaporte a left join clientes b on a.cliid=b.cliid left join clientesdir c on a.dircliid=c.dircliid left join transportistas d on a.traid=d.traid left join gasequipos e on a.equipidt=e.equipid left join transplacas f on a.plaid=f.plaid left join transchofer g on a.choid=g.choid ";
  $xsql ="Select a.carid,a.cliid,b.nombre from cartaporte a, clientes b where a.cliid=b.cliid and carid=$id ";
$query = $this->db->query($xsql);
  if ($query) {
    $respuesta = array(
        'error' => FALSE,            
...
/************************************************************************/
 public function cargacarta_post(){
  ...
  if ($query) {
    ...
    $choid = $row->choid;   
    $respuesta = array('error' => FALSE, 'choid' => $xsql);
    if ($choid>0){
    //  $xsql ="Select vehid, carid from vehiculos where carid>0 and choid=$choid";  
    //  $query = $this->db->query($xsql);
      $xsql ="Select carid from cartaporte where choid=$choid and impresa='X'";  
...
/************************************************************************/
  private function losdatos($tipo){
    $dat=json_decode($this->datap['datosm']);
    $dirid = $dat->dircliid;
    $id = $dat->cliid;
    $tcp3 = "N";
    ...
    $tcp = "N";
//    if ($tipo='A'){
      $xsql = "select tipocp3,tipocp4 from clientes where cliid=".$id;  
      $query = $this->db->query($xsql);
      if ($query) {
        $row = $query->row();
        $tcp3 = $row->tipocp3;  
...
/************************************************************************/
 ...
 // $this->response( $respuesta );
  ...
  //  $respuesta = array('error' => TRUE, 'carid' => 0);
    $pasa = 1;
    $xsql = "select year(fechaenv) as anio,tipofactcp,numfac from cartaporte WHERE carid='".$carid."'";
    $query = $this->db->query($xsql);
    $row = $query->row();
  ...
  if ($carid>0){
    ...
    if ($tipofactcp=="F" || $tipofactcp=="T"){
       ...
       if ($numfac==0){
         ...
         $this->facidcp=$this->GeneraFact->genfactura($carid,"cartacros");
       }
    } 
    $respuesta = array('error' => FALSE, 'carid' => $carid, 'facidcp'=>$this->facidcp, 'xsql'=>$xsql);
  } 
  $this->response( $respuesta );
}  
