...
/************************************************************************/
 public function cpemb2_post(){
  ...
  $xsql = "select a.carid, a.fecha, a.destino, left(b.nombre,15) as nombre, left(c.nombre,12) as nombretrans, d.nombrechofer as chofer from cartaporte a, clientes b, transportistas c, transchofer d where a.cliid=b.cliid and a.traid=c.traid and a.choid=d.choid and a.tipo<>'PendC' and a.tipo<>'PendA' and a.destino='Embarque' and CAST(a.fecha AS DATE)='".$fecha."' and a.cliid=".$cliid;
  $query = $this->db->query($xsql);
  $respuesta = array('error' => TRUE,
    'items' => $query->result_array()
  );
...
/************************************************************************/
 public function cpalm2_post(){
  ...
  $fecha = date('Ymd', strtotime($f));
  $cliid=$this->datap['cliid'];
  $xsql = "select  a.carid, a.fecha, a.destino, left(b.nombre,15) as nombre, left(c.nombre,12) as nombretrans, d.nombrechofer as chofer from cartaporte a, clientes b, transportistas c, transchofer d where a.cliid=b.cliid and a.traid=c.traid and a.choid=d.choid and a.tipo<>'PendC' and a.tipo<>'PendA' and a.destino='Almacen' and CAST(a.fecha AS DATE)='".$fecha."' and a.cliid=".$cliid;
  $query = $this->db->query($xsql);
  $respuesta = array('error' => TRUE,
    'items' => $query->result_array()
  );
