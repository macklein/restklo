...
class Gastos extends REST_Controller {
  ...
  var $datap,$data,$id,$empid;  //Datos que se agregan solp cuando se dan de alta 
...
/************************************************************************/
  public function pago_post(){
    ...
    $formid = $data['formap'];
    $respuesta = array('error' => TRUE, 'gastid' => $gastid);
    $xsql = "Select empreid from gasformasp where formid=$formid";  
    $query = $this->db->query($xsql);
    if ($query) {
      $row = $query->row();
      $empreid = intval($row->empreid);
      $xsql="update gastos set fechapago='".$fecha."', pagada='S', formid='".$formid."', empreidp='".$empreid."' where gastid=".$gastid;
      $query = $this->db->query($xsql);
      if ($query){
        $respuesta = array('error' => FALSE, 'gastid' => $gastid);
      }  
