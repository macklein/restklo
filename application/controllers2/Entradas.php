...
/************************************************************************/
 ...
 public function index_post(){
    ...
    if ($estatus=="Pendientes"){
      $cond="and (entradas.estatus='Pend' or entradas.terminada='N') ";
    }else{
...
//$conduser='';
  ...
  $campos="entradas.entid, DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha , entradas.carnum, entradas.capturados, entradas.estatus, entradas.carid, entradas.umedida, entradas.upeso, entradas.conflete, entradas.cantidad, entradas.cliid, clientesmat.descripcion, clientes.nombre, cantidad-capturados as faltan, entradas.tipofactcp, entradas.serie, entradas.facidn, entradas.numfac, entradas.timbrada, entradas.impresa ";    
...
/************************************************************************/
 public function seleid_post(){
  ...
  $id=$this->datap['id'];
  $xsql ="Select entid, cliid, matcliid, fecha, estatus, userid as empid, equipidm, manid, asisid, empreid, conflete, umedida, traid, traid as traid2, choid, choid as choid2, dircliid, equipidt, plaid, plaid as plaid2, equipidr, capturados, cantidad-capturados as faltan, upeso, cantidad, ruta from entradas where entid=$id";  

//  $xsql ="Select entid, cliid, matcliid, fecha, estatus, userid as empid, equipidm, manid, asisid, empreid, conflete, umedida, traid, choid, dircliid, equipidt, plaid, equipidr, capturados, cantidad-capturados as faltan, upeso, cantidad, ruta from entradas where entid=$id";  
  $query = $this->db->query($xsql);
    if ($query) {
      $respuesta = array(
        'error' => FALSE,            
...
/************************************************************************/
  private function losdatos(){
    ...
    $xsql = "select tipocp6,tipocp7 from clientes where cliid=".$dat->cliid;  
    $query = $this->db->query($xsql);
    if ($query) {
      $row = $query->row();
      $tcp = $row->tipocp6;  
...
/************************************************************************/
 ...
 //   $cantidad=0;
    ...
    $xsql = "select dircliid from clientesdir where cliid=".$cliid;  
    $query = $this->db->query($xsql);
    if ($query) {
      $row = $query->row();
    ...
    $xsql = "select estatus,numfac,tipofactcp,moneda,ruta,serie,facidn from entradas WHERE entid=".$entid;
    $fecha=date("Y-m-d H:i:s");
    $query = $this->db->query($xsql);
    $row = $query->row();
    $estatus = $row->estatus;
    ...
    if ($tipodocs=="Pendientes" && $numfac==0){ 
     $this->db->trans_begin();
     $xsql = "UPDATE entradas SET dircliid='$dircliid', choid='$choid', conflete='$conflete', plaid='$plaid', fechaenv='$fecha', equipidm='$equipidm', equipidt='$equipidt', equipidr='$equipidr', manid='$manid', asisid='$asisid', traid='$traid', cliid='$cliid', matcliid='$matcliid', umedida='$umedida', upeso='$upeso', cantidad='$cantidad' WHERE entid='".$entid."'"; 
      $query = $this->db->query($xsql);
      if ($query){
        $xsql = "INSERT INTO cartaporte(fecha, fechaenv, cliid, matcliid, traid, destino, dircliid, choid, plaid, tipo, entid, empreid, userid, impresa, afactura, equipidm, equipidt, equipidr, manid, asisid, nvo, conflete, numfac, tipofactcp, moneda, ruta, serie, facidn) VALUES('$fecha', '$fecha', '$cliid', '$matcliid', '$traid', 'Almacen', '$dircliid', '$choid', '$plaid', 'Ent', $entid, $empreid, $empid, 'No', 'N', $equipidm, $equipidt, $equipidr, $manid, $asisid, 'S', '$conflete',  $numfac, '$tipofactcp', '$moneda', '$ruta', '$serie', $facidn);";
        $query = $this->db->query($xsql);
        if ($query) {
          ...
          if ($query){
            foreach ( $query->result() as $rw1 ) {              
              ...
              $query = $this->db->query($xsql0);
               if ($query){
                 $cardetid=$this->db->insert_id();  
                 $xsql0 = "INSERT INTO almacen (cardetid, entdetid, entid, carid, fecha, cantidad, peso, descrip1, descrip2, descrip3, descrip4, cliid, proid, matcliid, empreid, userid, estatus, tipo, nvo) VALUES ('$cardetid', '$entdetid', '$entid', '$carid', '$fecha', '$cantidad', '$peso', '$descrip1', '$descrip2', '$descrip3', '$descrip4', '$cliid', '$proid', '$matid', '$empreid', '$empid', 'Pend', 'Ent', 'S')";
                  $query = $this->db->query($xsql0);
               } 
            } 
    ...
    if ($carid>0){
      ...
      if ($tipofactcp=="F" || $tipofactcp=="T"){
          ...
          if ($numfac==0){
            ...
            $this->facidcp=$this->GeneraFact->genfactura($carid,"entrada");
          }
      } 
      $respuesta = array('error' => FALSE, 'carid' => $carid, 'facidcp'=>$this->facidcp, 'tipofactcp'=>$tipofactcp,'xsql'=>$xsql);
    } 
