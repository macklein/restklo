...
//public function genfactura_post(){
  ...
  public function genfactura($carid,$file){
//    $this->datap = $this->post();
    $facid=0;
    $cliid=0;
    $this->facidcp=0;
...
/************************************************************************/
  public function enviarcarta_post(){
    //Esta CartaPorte es solo de CrossDock-Almacen, CrossDock-Embarque
    $data = $this->post();
    $this->facidcp=0;
    $xsql="Prueba xxxxxx";
 ...
 // $this->response( $respuesta );
    ...
    $tipocp='N';
    $xsql = "select $n as tipocp from clientes where cliid=".$id;  
    $query = $this->db->query($xsql);
    if ($query) {
      $row = $query->row();
      $tipocp = $row->tipocp;  
...
// $this->response( $respuesta );
  ...
  if ($carid>0){
    if ($tipocp=='F' || $tipocp=='T'){
      $this->genfactura($carid,'cartaporte')
    }
