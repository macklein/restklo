...
/************************************************************************/
 public function cpemb2_post(){
  ...
  $xsql = "select a.carid, a.fecha, a.destino, left(b.nombre,15) as nombre, c.nombre as nombretrans, d.nombrechofer as chofer from cartaporte a, clientes b, transportistas c, transchofer d where a.cliid=b.cliid and a.traid=c.traid and a.choid=d.choid and a.tipo<>'PendC' and a.tipo<>'PendA' and a.destino='Embarque' and year(fecha) = '$anio' and month(fecha)= '$mes' and a.cliid=".$cliid;
  $query = $this->db->query($xsql);
  $respuesta = array('error' => TRUE,
    'items' => $query->result_array()
  );
