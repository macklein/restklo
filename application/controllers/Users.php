...
/************************************************************************/
 public function login_post(){
  ...
  //  $xsql = "SELECT * FROM users WHERE email='".$email."' and password='".$pass."'";
    ...
    $xsql = "SELECT  user_id, fullname, user_name, user_email, user_password_hash, cliid, cliid2, ctesids, transids, proid, traid, choid, tipouser, modoop FROM users WHERE user_name = '" . $email . "' OR user_email = '" . $email . "';";
...
if ($rows==1){
    ...
    //  $token = utf8_encode(password_hash($mytok,  PASSWORD_DEFAULT));
      ...
      $xsql = "INSERT INTO usersesion SET UserId=$row->user_id, UserName='$row->user_name', IpLocal='found', FechaHora='$fecha', MyToken='$mytok', vuexid='$userid', Estatus='Actual', nompc='$nompc', tcpip='$localIP', nvo='S' "; //, Token='".$token."'";
      $query = $this->db->query($xsql);
      $sesid=$this->db->insert_id();
      if ($sesid>0){
      //  $xsql = "INSERT INTO usersesion SET UserId=$row->user_id, UserName='$row->user_name', IpLocal=$sesid, FechaHora='$fecha', MyToken='$mytok', vuexid='$userid', Estatus='Actual', nompc='$nompc', tcpip='$localIP', nvo='S' "; //, Token='".$token."'";
      //  $query = $this->db->query($xsql);
...
public function debug_post(){
  ...
  $choid = $data['choid'];    
  $datos="re=$myrest es=$status se=$sesid us=$user_id na=$user_name cl=$cliid tr=$transids t2=$traid ti=$tipouser ch=$choid";

  $xsql = "INSERT Into datos SET dato='$datos' ";
  $query = $this->db->query($xsql);
...
/********************************************************************* */
 public function index_post(){
    ...
    $tables = "users";
    $campos = "user_id, fullname, user_name, user_email, tipouser, emailreal, enviar, cliid, proid, ctesids, traid, transids, choid ";
...
/************************************************************************/
 public function email_post(){
  $this->datap = $this->post();
  $email=$this->datap['user_email'];
  $xsql ="Select user_id, user_email from users where user_email='$email'";  
...
'user_password_hash'=>$user_password_hash);
  ...
  private function losdatos(){
    ...
    $this->data = array(        
        'user_name'=>strtoupper($dat->user_name),
        'ctesids'=>$dat->ctesids,
     ...
     //   'email'=>$dat->email,
        ...
        'emailreal'=>$dat->emailreal,
        'fullname'=>strtoupper($dat->fullname),
        'cliid'=>$dat->cliid,        
        'cliid2'=>$dat->cliid2,
        'traid'=>$dat->traid,
        'enviar'=>$dat->enviar);
