...
/************************************************************************/
 public function login_post(){
    ...
    //  $token = utf8_encode(password_hash($mytok,  PASSWORD_DEFAULT));
      ...
      $xsql = "INSERT INTO usersesion SET UserId=$row->user_id, UserName='$row->user_name', FechaHora='$fecha', MyToken='$mytok', vuexid='$userid', Estatus='Actual', nompc='$nompc', tcpip='$localIP' "; //, Token='".$token."'";
...
/********************************************************************* */
 public function lasturl_post(){
    ...
    if ($userid>0){
      $xsql = "UPDATE users SET lasturl='$lasturl', estatus='$estatus' WHERE user_id=".$userid;
      $query = $this->db->query($xsql);
...
/********************************************************************* */
 public function sesionseek_post(){
  ...
  $xsql = "SELECT * FROM usersesion WHERE vuexid='".$vuexid."' and sesid=".$sesionid." and Estatus='Actual' "; 
  $respuesta = array("status" => "noFound");
  $query1 = $this->db->query($xsql);
  $rows = $query1->num_rows();
  if ($rows==1){
