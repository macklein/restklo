  ...
  public function cartaporte_post(){
    ...
    if (($tipodocs=="Pendientes") and ($estatus<>"Term")){ 
     ...
     $xsql = "UPDATE entradas SET dircliid='$dircliid', choid='$choid', conflete='$conflete', plaid='$plaid', fechaenv='$fecha', equipidm='$equipidm', equipidt='$equipidt', equipidr='$equipidr', manid='$manid', asisid='$asisid', traid='$traid', cliid='$cliid', matcliid='$matcliid', umedida='$umedida', upeso='$upeso', cantidad='$cantidad' WHERE entid='".$entid."'"; 
      $query = $this->db->query($xsql);
      if ($query){
        $xsql = "INSERT INTO cartaporte(fecha, fechaenv, cliid, matcliid, traid, destino, dircliid, choid, plaid, tipo, entid, empreid, userid, impresa, afactura, equipidm, equipidt, equipidr, manid, asisid) VALUES('$fecha', '$fecha', '$cliid', '$matcliid', '$traid', 'Almacen', '$dircliid', '$choid', '$plaid', 'Ent', $entid, $empreid, $empid, 'No', 'N', $equipidm, $equipidt, $equipidr, $manid, $asisid);";
        $query = $this->db->query($xsql);
        if ($query) {
          ...
          if ($query){
            foreach ( $query->result() as $rw1 ) {              
              ...
              $xsql0 = "INSERT INTO cartaportedet(fecha, entdetid, entid, carid, cliid, proid, cantidad, descrip1, descrip2, descrip3, descrip4, destino, peso, matcliid, estatus, empreid, userid, estatus2) VALUES('$fecha', '$entdetid', '$entid', '$carid', '$cliid', '$proid', '$cantidad', '$descrip1', '$descrip2', '$descrip3', '$descrip4', 'Almacen', '$peso', '$matid', 'Ent', $empreid, $empid, 'Pend');";
...
/************************************************************************/
 public function cargardet_post(){
  ...
  public function agregardet_post(){
    ...
    $xsql = "Insert into entradasdet (entid, fecha, cliid, proid, descrip1, descrip2, descrip3, descrip4, peso, matcliid, estatus, cantidad, empreid, userid) Values('$entid', '$fecha', '$cliid', '$proid', '$descr1', '$descr2', '$descr3', '$descr4', '$peso', '$matid', 'Pend', '$cantidad', '$empreid', '$empid')";
    $query = $this->db->query($xsql);
    if ($query){
