...
/************************************************************************/
 ...
 public function carga_post(){  
    ...
    $cliid = $data['cliid'];
    $empreid = 0;
    $xsql = "Select a.empreid,b.matcliid,b.descripcion from clientes a, clientesmat b where a.cliid=b.cliid and a.cliid=$cliid";
    $query = $this->db->query($xsql);
    if ($query) {
        $numr=$query->num_rows();
        if ($numr>0){
...
/************************************************************************/
 public function cargaid_post(){  
    $data = $this->post();
    $matid = $data['matid'];
    $xsql = "Select a.unidadmed, a.unidadpes, a.descrip1, a.descrip2, a.descrip3, a.descrip4, a.capcantidad, b.empreid from clientesmat a, clientes b where a.cliid=b.cliid and matcliid=$matid";
    $query = $this->db->query($xsql);
    if ($query) {
      $numr=$query->num_rows();
      if ($numr>0){
