            ...
            if ($destino=="Almacen" || $destino=="Almacen2" || $destino=="Transitorio"){
              ...
              foreach ( $query1->result() as $rw0 ) {
               ...
               // $carid = $rw0->CarId;
                ...
                $xsql .= " (cardetid, carid, fecha, crosdetid, cantidad, peso, descrip1, descrip2, descrip3, descrip4, cliid, proid, crosid, matcliid, empreid, userid, estatus, carnum, tipo) VALUES ('$cardetid', '$carid', '$fecha', '$crosdetid', '$cantidad', '$peso', '$descrip1', '$descrip2', '$descrip3', '$descrip4', '$cliid', '$proid', '$crosid', '$matcliid', '$empreid', '$userid', 'Pend', '$carnum', 'Cros')"; 
 ...
 public function cargacli_post(){
  ...
  public function agregardet_post(){
    ...
    if ($cardetid>0){
      ...
      $xsql = "INSERT INTO cartaportedet (carid, fecha, crosdetid, cantidad, peso, descrip1, descrip2, descrip3, descrip4, cliid, proid, crosid, matcliid, empreid, userid, estatus2, carnum) VALUES ('$carid', '$fecha', '$crosdetid', '$cantidad', '$pesoreal', '$descr1', '$descr2', '$descr3', '$descr4', '$cliid', '$proid', '$crosid', '$matcliid', '$empreid', '$userid', 'Pend', '$carnum')";
      $query = $this->db->query($xsql);
    }
    $entro="No";
    if ($query){
