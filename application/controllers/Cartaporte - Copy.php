...
//$conduser='';
  ...
  $campos="cartaporte.carid, DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha , cartaporte.destino, cartaporte.impresa, cartaporte.tipo, left(clientes.nombre,15) as nombre, left(transportistas.nombre,15) as nombretrans, transchofer.nombrechofer as chofer, cartaporte.tipofactcp, cartaporte.facidn, cartaporte.numfac ";
...
//    $this->datap = $this->post();
 ...
 //   $file=$this->datap['file'];  //cartaporte, entrada, otrosmovs, fleteloc
    ...
    $error=true;
    if ($facid>0 && $flag=1){
      $xsql = "Update cartaporte set NumFac=$facid Where carid=$carid";
      $query = $this->db->query($xsql);
      if ($query) {
        $this->facidcp=$facid;
        $error=false;
 ...
 //   $this->response( $respuesta );
  ...
  /*
  public function genfacturaNO_post(){
    $this->datap = $this->post();
    $facid=0;
    $cliid=0;
    $carid=intval($this->datap['carid']);
    ...
    $error=true;
    if ($facid>0 && $flag=1){
      $xsql = "Update cartaporte set facidcp=$facid Where carid=$carid";
      $query = $this->db->query($xsql);
      if ($query) {
        $error=false;
      }
