...
/************************************************************************/
 ...
 //  $xsql = "SELECT a.vehid,a.descrip,lat,lng FROM vehiculos a JOIN (SELECT MAX(hisid) hid, vehid FROM vehistoria GROUP BY vehid) c_max ON (c_max.vehid = a.vehid) JOIN vehistoria cd ON (cd.hisid = c_max.hid)";
   ...
   $xsql = "SELECT a.vehid,a.descrip,a.imei,cd.latitude,cd.longitude,cd.speed,cd.fecha FROM vehiculos a JOIN (SELECT MAX(rupid) rid, imei FROM ruptela GROUP BY imei) c_max ON (c_max.imei = a.imei) JOIN ruptela cd ON (cd.rupid = c_max.rid)";
