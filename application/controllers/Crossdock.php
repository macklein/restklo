...
/************************************************************************/
 ...
 public function index_post(){
  ...
  $campos="crossdock.crosid, DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha, clientes.nombre, left(clientesmat.descripcion,20) as descripcion, crossdock.carnum, crossdock.cantidad, crossdock.cantidad-crossdock.capturados as faltanc, crossdock.cantidad-crossdock.capturados as faltane, crossdock.capturados, crossdock.enviados ";
  ...
  public function cartaporte_post(){
    ...
    if (($tipodocs=="Pendientes") and ($estatus<>"Term")){ 
      ...
      if ($query){
        ...
        $plaid=0; 
        $fecha=date("Y-m-d H:i:s");            
        $xsql = "INSERT INTO cartaporte(fecha, cliid, traid, destino, dircliid, choid, plaid, tipo, crosid, empreid, userid, equipidm, manid, asisid) VALUES('$fecha', '$cliid', '$traid', '$desid', '$dircliid', '$choid', '$plaid', 'Carr', $crosid, $empreid, $empid, $equipidm, $manid, $asisid);";
        $query = $this->db->query($xsql);
        if ($query) {
         ...
         if ($query){
            foreach ( $query->result() as $row ) {              
               ...
               $xsql = "INSERT INTO cartaportedet(fecha, cardocid, cardocdetid, carid, cliid, cantidad, estatus, empreid, userid, peso, doctraid) VALUES('$fecha', '$crosid', '$cardocdetid', '$carid', '$cliid', '$cantidad', 'Car', $empreid, $empid, $peso, $doctraid)";
               $query = $this->db->query($xsql);
            } 
          } 
        }
...
/************************************************************************/
  public function enviacros_post(){
    ...
    $xsql = "UPDATE crossdock SET enviado='Si', estatus='Term', fecha='$fecha', fechaenv='$fecha' WHERE crosid='".$crosid."'";
    $query = $this->db->query($xsql);
    if ($query){
      $respuesta = array('error' => FALSE, 'carid' => $carid);
    }
...
//     $respuesta = array('error' => TRUE, "message" => $xsql);
  ...
  public function agregardet_post(){
    ...
    $xsql = "Insert into crossdockdet (crosid, fecha, cliid, proid, descrip1, descrip2, descrip3, descrip4, peso, matcliid, estatus, cantidad, empreid, userid) Values('$crosid', '$fecha', '$cliid', '$proid', '$descr1', '$descr2', '$descr3', '$descr4', '$peso', '$matid', 'Pend', '$cantidad', '$empreid', '$empid')";
    $query = $this->db->query($xsql);
    if ($query){
