...
/************************************************************************/
 public function cargacli_post(){
  ...
  $xsql = "select doctraid, date(fecha) as fecha, traid, pedimento, CAST(peso as UNSIGNED) as peso, piezas, descargadas, piezas-descargadas as pendientes, piezas-descargadas as cantidad from doctrailers where piezas>descargadas and estatus='Pend' and cliid=".$cliid;
  $query = $this->db->query($xsql);
    if ($query) {
      $respuesta = array(
        'error' => FALSE,            
