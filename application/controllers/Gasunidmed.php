...
class Gasunidmed extends REST_Controller {
...
/************************************************************************/
 public function index_post(){
    ...
    $tables = "gasunidmed ";
    $campos = "unidid, descripunid ";
    $sWhere = "descripunid LIKE '%".$buscar."%'"; 
    $order = " Order by unidid ";
...
$xsql = "Select unidid,descripunid from gasunidmed where $sWhere $order LIMIT $offset,$perPage";
...
/************************************************************************/
 public function carga_post(){
    $xsql = "Select unidid,descripunid from gasunidmed order by unidid";
    $query = $this->db->query($xsql);
    if ($query) {
        foreach ( $query->result() as $row ) {
            $records[] = array( 'label' => $row->descripunid, 'value' => $row->unidid );
        } 
      $respuesta = array(
        'error' => FALSE,            
        'records' => $records
...
/************************************************************************/
 public function alta_post(){
  ...
  $this->losdatos();
  $this->data += [ "nvo" => "S" ];
  $this->db->insert('gasunidmed',$this->data);
...
/************************************************************************/
 public function modif_post(){
  ...
  $id=$this->datap['id'];
  $this->losdatos();
  $this->db->where( 'unidid', $id );
  $hecho = $this->db->update( 'gasunidmed', $this->data);
  if ($hecho){
      $respuesta = array(
        'error' => FALSE,
        'id' => $this->id);
...
/************************************************************************/
  private function losdatos(){
    ...
    $this->data = array(
        'descripunid'=>strtoupper($dat->descripunid)
    );
  }
