...
//$cond="cartaporte.carid>0 ";
    ...
    if ($estatus=="Pendientes"){
      $cond="and (cartaporte.tipo='PendA' or cartaporte.terminada='A' or cartaporte.terminada='T') ";
    }else{
...
/************************************************************************/
 public function seleid_post(){
  ...
  $id=$this->datap['id'];
  $xsql ="Select carid, cliid, tipoalm, carnum, destino, fecha, dircliid, traid, equipidt, equipidr, equipidtm, equipidrm, choid, plaid, placa, equipidm, manid, asisid, empreid, userid as empid, ruta, tipofactcp, facidn, numfac from cartaporte where carid=$id";  
  $query = $this->db->query($xsql);
//  $records[] = array( 'label' => 'PLAC123', 'value' => 1 );
 ...
 //$records[] = array( 'label' => "123R", 'value' => 1 );
      $respuesta = array(
              'error' => FALSE,      
              'placas' => $records,   
...
/************************************************************************/
  private function losdatos(){
    ...
    $xequipt = $dat->equipidt;
    $xequipr = $dat->equipidr;
    $xplaid = $dat->plaid;
    $xequiptm = $dat->equipidtm;
    $xequiprm = $dat->equipidrm;   
    $xplaidm = $this->datap['placa'];
...
/************************************************************************/
  public function enviarcarta_post(){
    ...
    $equipidtm = $dat->equipidtm;
    $equipidrm = $dat->equipidrm;
    $placa = $dat->placa;
    ...
    $pasa = 1;
    $xsql = "select year(fechaenv) as anio,tipofactcp,tipocp,numfac from cartaporte WHERE carid=$carid";
    $query = $this->db->query($xsql);
    $row = $query->row();
    $anio = intval($row->anio);
    $numfac = intval($row->numfac);
...
// $this->response( $respuesta );
    ...
    if ($anio==0 && $numfac==0){  
      $this->db->trans_begin();
      if ($tipocp==4){
        $xsql = "UPDATE cartaporte SET dircliid='$dircliid', terminada='T', destino='$destino', traid='$traid', choid='$choid', plaid='$plaid', placa='$placa', fechaenv='$fecha', tipo='$tipoc', equipidt='$equipidt', equipidr='$equipidr', equipidm='$equipidm', equipidtm='$equipidtm', equipidrm='$equipidrm', manid='$manid',  empreid='$empreid', asisid='$asisid' WHERE carid='".$carid."'";
      }else{
        $xsql = "UPDATE cartaporte SET dircliid='$dircliid', terminada='T', destino='$destino', traid='$traid', choid='$choid', plaid='$plaid', placa='$placa', fechaenv='$fecha', tipo='$tipoc', equipidt='$equipidt', equipidr='$equipidr', equipidm='$equipidm', equipidtm='$equipidtm', equipidrm='$equipidrm', manid='$manid',  empreid='$empreid', asisid='$asisid' WHERE carid='".$carid."'";
      }
      $query = $this->db->query($xsql);
...
public function GenTrailers($carid,$tipocp){
  ...
  if ($id>0){
      ...
      $xsql = "update doctrailers set carid='$carid', pedimento='Cp-$carid', peso='$tpes', piezas='$tcan', umedida='$umedida', upeso='$upeso', matid='$matid', descrip='$descrip' Where doctraid='$id' ";
